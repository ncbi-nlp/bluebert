...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    def _one_layer(name, input_layer, output_size):
        input_size = input_layer.shape[-1].value

        weights = tf.get_variable(
            "{}_weights".format(name), [output_size, input_size],
            initializer=tf.contrib.layers.xavier_initializer())

        bias = tf.get_variable(
            "{}_bias".format(name), [output_size], initializer=tf.constant_initializer(0.01))

        with tf.variable_scope("{}_loss".format(name)):
            if is_training:
                input_layer = tf.nn.dropout(input_layer, keep_prob=0.5)
            output = tf.matmul(input_layer, weights, transpose_b=True)
            output = tf.nn.bias_add(output, bias)
            return output

    # 256
    # output_layer = _one_layer('layer1', output_layer, 256)
    # 128
    # output_layer = _one_layer('layer2', output_layer, 128)
    # 64
    # output_layer = _one_layer('layer3', output_layer, 64)
    ...
    output_bias = tf.get_variable(
        "output_bias", [1], initializer=tf.constant_initializer(0.01))
...
def main(_):
    ...
    if FLAGS.do_test:
        ...
        with tf.gfile.GFile(output_test_file, "w") as writer:
            ...
            for key in sorted(result.keys()):
                if key in ('MSE', 'loss', 'pearson', 'spearman'):
                    tf.logging.info("  %s = %s", key, str(result[key]))
                writer.write("%s = %s\n" % (key, str(result[key])))
